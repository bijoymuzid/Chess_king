...
# Disable PyAutoGUI failsafe so mouse can go to screen corners without raising exception
...
class StockfishBot(multiprocess.Process):
    ...
    def make_move(self, move):  # sourcery skip: extract-method
        ...
        # Ensure the window is frontmost and mouse is focused on the board
        # by adding a small pause and using multiple small drags
        pyautogui.moveTo(start_pos[0], start_pos[1])
        time.sleep(0.05 + self.mouse_latency)

        # Use click + drag with more reliable timing for Chrome
        pyautogui.mouseDown()
        time.sleep(0.03)
